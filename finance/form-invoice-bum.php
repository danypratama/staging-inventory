            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form action="proses/proses-invoice-bum.php" method="POST" id="myForm">
                        ...
                        include "koneksi.php";
                        $thn  = date('Y');
                        $sql  = mysqli_query($connect, "SELECT max(no_inv) as maxID, STR_TO_DATE(tgl_inv, '%d/%m/%Y') AS tgl FROM inv_bum WHERE YEAR(STR_TO_DATE(tgl_inv, '%d/%m/%Y')) = '$thn' ORDER BY no_inv ASC");
