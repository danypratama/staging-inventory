    ...
    if(isset($_POST['diambil-oleh'])){
        ...
        try{
            ...
            if($update_bukti_terima -> num_rows > 0){
                ...
                $update_inv_bum = mysqli_query($connect, "UPDATE inv_bum SET status_transaksi = 'Diterima' WHERE id_inv_bum = '$id_inv'");

                if ($update_bukti_terima && $update_inv_penerima && $update_inv_bum) {
                ...
                if ($simpan_bukti_terima && $update_inv_penerima && $update_inv_bum) {
                    // Commit transaksi jika berhasil
                    mysqli_commit($connect);
                    ?>
                        <!-- Sweet Alert -->
