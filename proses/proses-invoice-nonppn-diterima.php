...
if(isset($_POST['diterima_driver'])){
    ...
    if($diterima_oleh == 'Customer'){
        ...
        try{
            ...
            $update_status_kirim = mysqli_query($connect, "UPDATE status_kirim SET dikirim_ekspedisi = '$nama_ekspedisi', resi = '$resi'  WHERE id_inv = '$id_inv'");

            if ( $bukti_terima && $query_update_inv) {
                // Commit transaksi
                $connect->commit();
                header("Location:../invoice-reguler-dikirim.php?sort=baru");
            }
